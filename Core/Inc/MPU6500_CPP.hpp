...
#ifndef MPU6500_CPP_H
#define MPU6500_CPP_H
...
class MPU6500 final : public IMUBase {
public:
	MPU6500() = default;
	...
	int setIMUGeometry(const int index) override { geometryIndex = index; return 0; };
	...
	std::string IMUName() override {
		return "MPU-6500";
	}
	std::string IMUType() override {
		return "MPU6500";
	}
	std::string IMUManufacturer() override {
		return "InvenSense";
	}
private:
	...
	static void writeByte(const uint8_t address, const uint8_t subAddress, uint8_t data)
	{
		HAL_I2C_Mem_Write(&hi2c3, address, subAddress, I2C_MEMADD_SIZE_8BIT, &data, 1, 1);
	}

	static uint8_t readByte(const uint8_t address, const uint8_t subAddress)
	{
		uint8_t data;
		HAL_I2C_Mem_Read(&hi2c3, address, subAddress, I2C_MEMADD_SIZE_8BIT, &data, 1, 1);
		return data;
	}

	static void readBytes(const uint8_t address, const uint8_t subAddress, uint8_t count, uint8_t* dest)
	{
		HAL_I2C_Mem_Read(&hi2c3, address, subAddress, I2C_MEMADD_SIZE_8BIT, dest, count, 1);
	}

	bool dataAvailable() const{ return (readByte(IMUAddress, MPU6500_INT_STATUS) & 0x01);}
};
#endif //MPU6500_CPP_H